...
#############
## Fitting ##
phyloMDA_result_moderateT <- list()
time5 <- Sys.time() ## start time
for (i in 1: 1000){
  phyloMDA_result_moderateT[[i]] <- eBay_comps(otu_tab= y_moderateT_list[[i]], 
                                            prior = "zero-inflated-Dirichlet-tree",
                                            tree= COMBO_tree2, 
                                            model= "MIX")
}
time6 <- Sys.time() ## End time
phyloMDA_time_moderateT <- difftime(time6, time5, units = "secs")#; phyloMDA_time_moderateT
# save the result
save(phyloMDA_result_moderateT, file = "phyloMDA_result_moderateT.RData")
save(phyloMDA_time_moderateT, file = "phyloMDA_time_moderateT.RData")