...
#############
## Fitting ##
phyloMDA_result_largeT <- list()
time5 <- Sys.time() ## start time
for (i in 1: 1000){
  phyloMDA_result_largeT[[i]] <- eBay_comps(otu_tab= y_largeT_list[[i]], 
                                            prior = "zero-inflated-Dirichlet-tree",
                                            tree= COMBO_tree2, 
                                            model= "MIX")
}
time6 <- Sys.time() ## End time
phyloMDA_time_largeT <- difftime(time6, time5, units = "secs")#; phyloMDA_time_largeT
# save the result
save(phyloMDA_result_largeT, file = "phyloMDA_result_largeT.RData")
save(phyloMDA_time_largeT, file = "phyloMDA_time_largeT.RData")