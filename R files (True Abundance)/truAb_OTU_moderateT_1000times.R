...
#######################
## Scenarios 3: small T
simulated_combo_data2_moderateT <- true_abundance(sigma = 1, rho = 1, D_phylo = D_phylo_combo2, 
                                                  n= n, K = 62, Ni = ni_combo, Temp = 12, num_replicate = 1000)
# Save the simulated result
# save(simulated_combo_data2_moderateT, file = "simulated_combo_data2_moderateT.RData")

# True abundance
true_pi_moderateT <- simulated_combo_data2_moderateT$true_pi
# save the true abundance table
# save(true_pi_moderateT, file = "true_pi_moderateT.RData")

# OTU counts list
y_moderateT_list <- simulated_combo_data2_moderateT$true_otu
# save the OTU counts table
# save(y_moderateT_list, file = "y_moderateT_list.RData")

# Proportion of zeros
prop_zero_moderateT <- c()
for (i in 1: 1000){
  prop_zero_moderateT[i] <- signif(mean(y_moderateT_list[[i]] == 0), digits = 6)
}
biological_moderateT <- simulated_combo_data2_moderateT$delta_param
prop_bio_zero_moderateT<- signif(mean(biological_moderateT == 1), digits = 6)
# save the proportion of zeros
zero_moderateT <- list(prop_zero_moderateT, prop_bio_zero_moderateT)
# save(zero, file = "zero.RData")

# Remove columns of the OTU tables with all zeros
y_noColzero_moderateT <- list()
# Remove columns of the true abundance with all zeros
true_pi_noColzero_moderateT <- list()
for (i in 1: 1000){
  X <- y_moderateT_list[[i]]
  Z1 <- true_pi_moderateT
  ...
  y_noColzero_moderateT[[i]] <- X
  true_pi_noColzero_moderateT[[i]] <- Z1
}
# save the OTU counts table with no columns with all zeros
# save(y_noColzero_moderateT, file = "y_noColzero_moderateT.RData")
# save the true abundance table with no columns with all zeros
# save(true_pi_noColzero_moderateT, file = "true_pi_noColzero_moderateT.RData")


# Remove columns and rows of the OTU tables with all zeros
y_noColRowzero_moderateT <- list()
true_pi_noColRowzero_moderateT <- list()
for (i in 1: 1000){
  Y <- y_moderateT_list[[i]]
  Z2 <- true_pi_moderateT
  ...
  y_noColRowzero_moderateT[[i]] <- Y
  true_pi_noColRowzero_moderateT[[i]] <- Z2
}

# save the OTU counts table with no columns or rows with all zeros
# save(y_noColRowzero_moderateT, file = "y_noColRowzero_moderateT.RData")
# save the true abundance table with no columns or rows with all zeros
# save(true_pi_noColRowzero_moderateT, file = "true_pi_noColRowzero_moderateT.RData")

# Remove columns with less than two positive values (for zComposition package)
y_positive2value_moderateT <- list()
true_pi_positive2value_moderateT <- list()
for (i in 1:1000){
  X2 <- y_moderateT_list[[i]]
  R <- X2 > 0
  positive_counts <- colSums(R)
  y_positive2value_moderateT[[i]] <- X2[, positive_counts >= 2]
  true_pi_positive2value_moderateT[[i]] <- true_pi_moderateT[, positive_counts >= 2]
}
...
save.image(file = "truAb_OTU_moderateT_1000times.RData")