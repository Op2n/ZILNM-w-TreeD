...
#######################
## Scenarios 3: small T
simulated_combo_data2_largeT <- true_abundance(sigma = 1, rho = 1, D_phylo = D_phylo_combo2, 
                                               n= n, K = 62, Ni = ni_combo, Temp = 30, num_replicate = 1000)
# Save the simulated result
# save(simulated_combo_data2_largeT, file = "simulated_combo_data2_largeT.RData")

# True abundance
true_pi_largeT <- simulated_combo_data2_largeT$true_pi
# save the true abundance table
# save(true_pi_largeT, file = "true_pi_largeT.RData")

# OTU counts list
y_largeT_list <- simulated_combo_data2_largeT$true_otu
# save the OTU counts table
# save(y_largeT_list, file = "y_largeT_list.RData")

# Proportion of zeros
prop_zero_largeT <- c()
for (i in 1: 1000){
  prop_zero_largeT[i] <- signif(mean(y_largeT_list[[i]] == 0), digits = 6)
}
biological_largeT <- simulated_combo_data2_largeT$delta_param
prop_bio_zero_largeT<- signif(mean(biological_largeT == 1), digits = 6)
# save the proportion of zeros
zero_largeT <- list(prop_zero_largeT, prop_bio_zero_largeT)
# save(zero, file = "zero.RData")

# Remove columns of the OTU tables with all zeros
y_noColzero_largeT <- list()
# Remove columns of the true abundance with all zeros
true_pi_noColzero_largeT <- list()
for (i in 1: 1000){
  X <- y_largeT_list[[i]]
  Z1 <- true_pi_largeT
  ...
  y_noColzero_largeT[[i]] <- X
  true_pi_noColzero_largeT[[i]] <- Z1
}
# save the OTU counts table with no columns with all zeros
# save(y_noColzero_largeT, file = "y_noColzero_largeT.RData")
# save the true abundance table with no columns with all zeros
# save(true_pi_noColzero_largeT, file = "true_pi_noColzero_largeT.RData")


# Remove columns and rows of the OTU tables with all zeros
y_noColRowzero_largeT <- list()
# Remove columns and rows of the true abundance with all zeros
true_pi_noColRowzero_largeT <- list()
for (i in 1: 1000){
  Y <- y_largeT_list[[i]]
  Z2 <- true_pi_largeT
  ...
  y_noColRowzero_largeT[[i]] <- Y
  true_pi_noColRowzero_largeT[[i]] <- Z2
}

# save the OTU counts table with no columns or rows with all zeros
# save(y_noColRowzero_largeT, file = "y_noColRowzero_largeT.RData")
# save the true abundance table with no columns or rows with all zeros
# save(true_pi_noColRowzero_largeT, file = "true_pi_noColRowzero_largeT.RData")

# Remove columns with less than two positive values (for zComposition package)
y_positive2value_largeT <- list()
true_pi_positive2value_largeT <- list()
for (i in 1:1000){
  X2 <- y_largeT_list[[i]]
  R <- X2 > 0
  positive_counts <- colSums(R)
  y_positive2value_largeT[[i]] <- X2[, positive_counts >= 2]
  true_pi_positive2value_largeT[[i]] <- true_pi_largeT[, positive_counts >= 2]
}
...
save.image(file = "truAb_OTU_largeT_1000times.RData")
